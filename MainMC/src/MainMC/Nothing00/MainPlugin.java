...
public class MainPlugin extends JavaPlugin {
	...
	public void onEnable() {
		...
		DataBase sql = new DataBase(Sql.getDatabase().getAbsolutePath());
		sql.open();
	...
	public static void reloadPlugin() {
		...
		for (Player p : plugin.getServer().getOnlinePlayers()) {
			User.userhash.get(p.getName()).saveTime();
		}
		User.userhash.clear();
		...
		for (Player p : plugin.getServer().getOnlinePlayers()) {
			User.userhash.put(p.getName(), new User(p.getName()));
			User.userhash.get(p.getName()).setJoinDate();
		}
