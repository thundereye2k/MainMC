...
public class Economy {
	...
	public void addMoney() {
		if (!this.addon) {
			Conf config = new Conf();
			Sql sql = new Sql(config.getDBName());
			sql.setBalance(this.p, getBalance() + this.money);
		} else {
			EconomyAPI eco = new EconomyAPI(this.p, this.money);
			eco.addMoney();
		}
	...
	public void setMoney() {
		if (!this.addon) {
			Conf config = new Conf();
			Sql sql = new Sql(config.getDBName());
			sql.setBalance(this.p, this.money);
		} else {
			EconomyAPI eco = new EconomyAPI(this.p, this.money);
			eco.setMoney();
		}
	...
	public void takeAll() {
		if (!this.addon) {
			...
			for (User user : User.getUsers()) {
				double m = 0;
				if (sql.getBalance(user.getName()) - this.money >= 0)
					m = sql.getBalance(user.getName()) - this.money;

				sql.setBalance(user.getName(), m);
			}
		} else {
			EconomyAPI eco = new EconomyAPI(this.p, this.money);
			eco.takeAll();
	...
	public void giveAll() {
		if (!this.addon) {
			...
			Sql sql = new Sql(config.getDBName());
			for (User user : User.getUsers()) {
				sql.setBalance(user.getName(), sql.getBalance(user.getName()) + this.money);
			}
		} else {
			EconomyAPI eco = new EconomyAPI(this.p, this.money);
			eco.addAll();
	...
	public void setAll() {
		if (!this.addon) {
			...
			Sql sql = new Sql(config.getDBName());
			for (User user : User.getUsers()) {
				sql.setBalance(user.getName(), this.money);
			}
		} else {
			EconomyAPI eco = new EconomyAPI(this.p, this.money);
			eco.setAll();
