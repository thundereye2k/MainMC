...
public class Kit extends KitData {
	...
	public void setDelay() {
		...
		if (super.getDelay() == 0)
			return;
		sql.setKitDelay(this.p.getUniqueId().toString(), this.kit.toLowerCase(),
				time.addTime(0, 0, 0, 0, 0, super.getDelay()));
	}

	public boolean hasDelay() {
		Conf config = new Conf();
		Sql sql = new Sql(config.getDBName());
		return sql.hasDelay(this.p.getUniqueId().toString(), this.kit.toLowerCase());
	}

	public String getExpire() {
		Conf config = new Conf();
		Sql sql = new Sql(config.getDBName());
		return sql.getDelay(this.p.getUniqueId().toString(), this.kit.toLowerCase());
	}

	public void expired() {
		Conf config = new Conf();
		Sql sql = new Sql(config.getDBName());
		sql.deleteKit(this.p.getUniqueId().toString(), this.kit.toLowerCase());
	}
